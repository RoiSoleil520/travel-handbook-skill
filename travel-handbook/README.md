...
AI 按实际地点选择地图：中国内地使用高德（`amap`），国外使用 Google Maps（`google`）。可在攻略 JSON 顶层设置 `"mapProvider": "amap"`，每天、事件及具体景点、住宿、接送、航班起降机场也能单独覆盖；跨境行程不会统一改成一种地图。
...
[完整示例](examples/trip.json) 是“云南五日慢游”：大理 2 天＋丽江 3 天，含天气备选、角色切换和高德地图。景点背景来源见页面“行程总览”；2027 年 4 月 3—7 日及时间线仅作演示，住宿、交通与门票未预订。示例已填入核实到的公开票价，在费用预算附查询日期与官方来源；套票、基础费用和可选项目分开说明。生成自己的攻略时替换示例数据并重新核价，不照搬日期或订单状态。
